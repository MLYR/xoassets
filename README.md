...
- 投资展示：投资主页按总览 / 基金 / 股票 / 虚拟货币分模块展示统计和图表；资产趋势固定同图展示总览、股票、基金、虚拟货币四条线，并支持周 / 月 / 年维度切换，左轴金额单位为 k；每日收益区域可在收益日历和全持仓交易记录之间切换；每日收益日历、昨日收益、趋势图每日收益和收益贡献统一读取 `xo_investment_holding_daily_profit` 持仓每日收益表；基金、股票、虚拟货币模块内分别展示持仓表格、买入卖出、价格刷新和收益分析入口，持仓表格支持按持有市值、持有收益、今日收益、昨日收益和最新净值 / 当前价排序，默认持有市值倒序；单个持仓的收益汇总、交易记录和总市值 / 价格走势在 `/investments/holdings/:id` 查看；清仓持仓的持有收益展示已实现收益 + 当前浮动盈亏，总市值走势读取后端按历史头寸重建的 `chartPoints`，不能用当前 0 份额倒推历史。
