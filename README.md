...
- 移动端：已创建 uni-app 独立项目，复用后端 API；包含登录注册、首页仪表盘、快速记账、流水列表、账户管理、投资持仓和我的页面共 15 个页面，5 个底部 Tab；延续 Web 浅蓝金融 SaaS 风格，卡片化 + 大圆角 + 适配手指点击。当前已补齐主题系统和通用 UI 组件层，后续页面可以基于 `AppPage`、`AppCard`、`AppAmount`、`AppIcon`、`AppActionButton`、`AppSectionHeader`、`AppBottomTabs` 继续复刻原型。
