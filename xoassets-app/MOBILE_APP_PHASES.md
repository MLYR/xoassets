...
- `/settings` 提供设置页骨架，当前覆盖主题切换、登录状态展示、版本信息和退出登录能力。
- `(tabs)/profile` 提供“我的”页骨架，聚合账户、预算和设置入口；投资管理入口已移除。
...
- 首页总资产卡中的“资产详情”跳转 `/assets/detail`，展示账户余额和投资资产的聚合下钻；总资产以快照接口为准，不在移动端用账户资产和投资资产相加重算。
- 首页净资产文案统一显示为“账户净资产”。
- 首页最近记录“查看全部”跳转记账页本周统计视图：`/ledger?view=stats&period=week`。
