...
- 投资收益：模块持仓表展示持有市值、持有收益、今日收益 / 收益率、昨日收益、最新净值 / 当前价等字段，并支持这些表头排序，默认按持有市值倒序；收益基准缺失时展示 `--`，不使用其他字段伪造 0；持仓详情页展示当前市值、持仓数量、成本、今日收益、总收益、回本涨幅、价格快照和当前持仓交易记录；每日收益日历、昨日收益和趋势图每日收益读取后端 `xo_investment_holding_daily_profit` 口径，不从累计持有收益反算。
