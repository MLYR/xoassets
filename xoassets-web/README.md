...
- 已接入真实接口：登录、注册、账户管理。
- 暂用 mock 数据：首页仪表盘、记账流水、投资持仓、数据分析、AI 报告、预算管理、资产目标。
- 请求封装：`src/services/http.ts` 统一处理 `/api` baseURL、JWT Header、业务响应和 401 跳转。
- API 模块：`src/services/authApi.ts`、`src/services/accountApi.ts`。
...
- `src/services`：HTTP 请求封装、认证 API、账户 API 和暂未联调页面的 mock 服务。
- `src/types`：业务类型定义。
